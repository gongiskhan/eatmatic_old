...
(function( $ ) {
    ...
    function addListAppender(item, handler) {
        var appender = $('<div>', { 'class': 'item appender' }),
            btn      = $('<button></button>', { 'data-role': 'button','class': 'property genericBtn ui-link ui-btn ui-shadow ui-corner-all' });
    ...
    function construct(opt, json, root, path) {
        ...
        for (var key in json) {
            ...
            var item     = $('<div>',   { 'class': 'item', 'data-path': path }),
                property =   $(opt.propertyElement || '<input>', { 'class': 'property ui-input-text ui-shadow-inset ui-body-inherit ui-corner-all ui-textinput-autogrow' }),
                value    =   $(opt.valueElement || '<input>', { 'class': 'value ui-input-text ui-shadow-inset ui-body-inherit ui-corner-all ui-textinput-autogrow'    }),
                deleteEl    =   $('<a>', { 'data-role':'button', 'class': 'genericBtn deleteItemButton ui-link ui-btn ui-shadow ui-corner-all'    });
                currencyEl    =   $('<span>', { 'class': 'currency' });

            if (isObject(json[key]) || isArray(json[key])) {
                addExpander(item);
            }
            
            item.append(property).append(value).append(currencyEl).append(deleteEl);;
            root.append(item);
            
            property.val(key).attr('title', key);
            var val = stringify(json[key]);
            value.val(val).attr('title', val);
            deleteEl.html('X').val('X').attr('title', 'X');
            currencyEl.html('&euro;').val('&euro;').attr('title', '&euro;');
